        ...
        <table class="list" >
            <tr>
				...
				<#list pageVoList as it>
					<tr class="odd">
                        <td>
                            <a href="/page/create?jobId=${job.id}&pageId=${it.id}">${it.name}</a>
                        </td>
                        <td>
							<#if it.patterns??>
                        ...
                        <td>
                            <a href="javascript:if(confirm('确认删除？页面：${it.name!}'))location.href='/page/delete?pageId=${it.id}&jobId=${job.id}'">删除</a>
                        </td>
                    </tr>
				</#list>
        </table>
