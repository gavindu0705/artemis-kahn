...
<div class="main-content">
    ...
    <div style="padding-left:5px; font-size:10px;">
        >>&nbsp;<a href="/job/stat?jobId=${job.id}&sessionId=${job.sessionId}">${job.name}</a>
			<#if page??>
				/&nbsp;<a href="/page/list?jobId=${job.id}">页面规则</a>&nbsp;/&nbsp;${page.name}
			<#else>
			 	/&nbsp;<a href="/page/list?jobId=${job.id}">页面规则</a>&nbsp;/&nbsp;创建
			</#if>
    </div>
        ...
        <form action="/page/save">
            <table>
                ...
                <tr>
                    <td>PASS PATTERN</td>
                    <td>
                        <textarea name="patterns" style="width:900px;height:80px;"><#if page?? && page.patterns??>${t_util.join(page.patterns!, '\r\n')}</#if></textarea>
                    </td>
                </tr>
                ...
                <tr>
                    <td>错误标签</td>
                    <td>
                        <textarea name="errTag" style="width:900px;height:80px;"><#if page?? && page.errTag??>${t_util.join(page.errTag!, '\r\n')}</#if></textarea>
                    </td>
                </tr>
                ...
                <tr>
                    <td>正确标签</td>
                    <td>
                        <textarea name="sucTag" style="width:900px;height:80px;"><#if page?? && page.sucTag??>${t_util.join(page.sucTag!, '\r\n')}</#if></textarea>
                    </td>
                </tr>
