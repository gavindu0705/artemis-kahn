...
(function($) {
    ...
    $.Autocompleter = function(input, options) {
        ...
        $input.bind(
                ($.browser.opera ? "keypress" : "keyup") + ".autocomplete",
            function(event) {
        ...
        function receiveData(q, data) {
            if (data && data.length && hasFocus) {
                ...
                select.show();
                if (options.addDistrictEnabled && typeof(hideAddDistrict) == "function") {
                    hideAddDistrict();
                }
            } else if (data.length == 0) {
                if (options.addDistrictEnabled && typeof(addDistrict) == "function") {
                    addDistrict();
                    hideResultsNow();
                }
            } else {
    ...
    $.Autocompleter.Select = function(options, input, select, config) {
        ...
        var listItems, active = -1, data, term = "", needsInit = true, element, list, autoBg;

        // Create results
        function init() {
            if (!needsInit)
                return;
            autoBg = $("<iframe style='z-index:100;position:absolute;border:0;'/>").appendTo(document.body);
            ...
            if (options.width > 0) {
                ...
                autoBg.css("width", options.width);
            }
        ...
        function movePosition(step) {
            ...
            $(input).val($(listItems[active]).data("ac_data").result)
        }
